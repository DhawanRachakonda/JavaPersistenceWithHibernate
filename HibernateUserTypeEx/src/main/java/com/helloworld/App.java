...
public class App {
	...
	private static void listItems(Session session) {
		...
		CriteriaQuery<Item> itemQuery = session.getCriteriaBuilder().createQuery(Item.class);
		itemQuery.from(Item.class);
		session.createQuery(itemQuery).getResultList().forEach(System.out::println);
	...
	public static void main(String[] args) {
		 ...
		 item.setMoney(new Money(new Currency("$"), 10.0));
		 session.save(item);
		 session.getTransaction().commit();*/
		 listItems(session);
	}
