...
public class App {
	...
	private static void linkCategoryItem(Session session) {
		session.getTransaction().begin();
		CategoryItem linkOne = new CategoryItem(session.find(Category.class, 1L), session.find(Item.class, 1L),session.find(User.class, 1L));
		CategoryItem linkTwo = new CategoryItem(session.find(Category.class, 1L), session.find(Item.class, 2L),session.find(User.class, 1L));
		CategoryItem linkThree = new CategoryItem(session.find(Category.class, 2L), session.find(Item.class, 1L),session.find(User.class, 2L));
